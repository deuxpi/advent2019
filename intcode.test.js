...
test('109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99 takes no input and produces a copy as itself as output', () => {
  const intcode = new Intcode('109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99')
  const outputs = []
  ...
  expect(outputs).toStrictEqual([109, 1, 204, -1, 1001, 100, 1, 100, 1008, 100, 16, 101, 1006, 101, 0, 99])
})

test('1102,34915192,34915192,7,4,7,99,0 should output a 16-digit number', () => {
  const intcode = new Intcode('1102,34915192,34915192,7,4,7,99,0')
  intcode.execute()
  expect(intcode.output).toBeGreaterThan(65535)
})

test('104,1125899906842624,99 should output the large number in the middle', () => {
  const intcode = new Intcode('104,1125899906842624,99')
  intcode.execute()
  expect(intcode.output).toBe(1125899906842624)
})
